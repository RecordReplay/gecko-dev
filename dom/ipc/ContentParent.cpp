...
ContentParent::MakePreallocProcess(const nsAString& aRecordingDispatchAddress) {
  RefPtr<ContentParent> process = new ContentParent(PREALLOC_REMOTE_TYPE, aRecordingDispatchAddress);
...
already_AddRefed<ContentParent> ContentParent::GetUsedBrowserProcess(
  ...
  // For simplicity, only use preallocated processes which are recording.
  if (!aRecordingDispatchAddress.Length()) {
    return nullptr;
  }
...
ContentParent::GetNewOrUsedLaunchingBrowserProcess(
  ...
  if (!Preferences::GetBool("devtools.recordreplay.disablePreallocated")) {
    contentParent = GetUsedBrowserProcess(aRemoteType, contentParents,
                                          maxContentParents, aPreferUsed, aPriority,
